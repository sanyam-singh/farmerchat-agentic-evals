...
analysis = json.load(open("/tmp/hindi_analysis.json"))
...
print(f"DONE recovered {len(recovered)} / 299 missing Hindi turns via direct re-query", flush=True)
...
json.dump(analysis, open("/tmp/hindi_analysis.json", "w"))
affected = sorted(set(analysis[tci]["test_code"] for tci, ri, ti, obs in recovered))
json.dump(affected, open("/tmp/requery_affected_hi.json", "w"))
